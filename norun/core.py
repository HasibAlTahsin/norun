...
def run_app(
    ...
    *,
    ...
    # load saved defaults if user didn't pass desktop
    if not desktop:
    ...
    # aggressive auto desktop on Wayland if still no desktop
    auto_desktop_applied = False
    if not desktop and vm.is_wayland_session():
        desktop = vm.recommend_virtual_desktop()
        auto_desktop_applied = True
        print(f"[yellow]Wayland detected -> auto virtual desktop enabled: {desktop}[/yellow]")

    # if user passed desktop and wants to persist it
